...
{
    [Binding]
    public class OPCStepDefinitions
    {
        ...
        public OPCStepDefinitions(IWebDriver driver)
        {
            this.driver= driver;
        }
        [Given(@"At First Go to the admin page for OPC plugin")]
        public void GivenAtFirstGoToTheAdminPageForOPCPlugin()
        {
            driver.Url = ConfigReader.GetInstance().GetBaseUrl();
        }

        [When(@"Click on the login Url for login as admin in OPC")]
        public void WhenClickOnTheLoginUrlForLoginAsAdminInOPC()
        {
            IWebElement btn = driver.FindElement(By.XPath("//a[contains(text(),'Account')]"));
            btn.Click();
        }

        [Then(@"Give email and pass for admin login in kingdom for OPC")]
        public void ThenGiveEmailAndPassForAdminLoginInKingdomForOPC()
        {
        ...
        [Then(@"Go to admin home before configure OPC items")]
        public void ThenGoToAdminHomeBeforeConfigureOPCItems()
        {
        ...
        [Then(@"For configure OPC")]
        public void ThenForConfigureOPC()
        {
            var opcConfig = new OnePageCheckout(driver);
            opcConfig.forbtnOPC();
            opcConfig.forbtnConfigurationforOPC();
            opcConfig.forCheckEnableOPC();
            opcConfig.forbtnSaveOPC();
            Thread.Sleep(3000);
